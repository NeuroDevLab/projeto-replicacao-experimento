...
#  rstatix: para fazer calculos de ANOVA, Kruskal, Wilcox e t. Nesse projeto, a Test t
...
# Optamos, por hora, em usar NÃO usar pool de animais.
...
# Nessa pasta iremos baixar o CSV do JSR para fazer a verificação do gráfico.
dest_folder <- "data/"

# E daremos um nome ao arquivo. Vamos salvar o CSV como objeto direto do R, por
# isso a extensão rds
sjrcsv <- 'sjr.rds'

# Precisamos criar a pasta, mas somente se ela não existir. Simples isso, mas
# em todas as linguagens de programaçao (o R é uma linguagem de muito alto nivel)
# você precisa dizer o óbvio. Se não tem cria. Se não fizermos isso, o R vai dar
# exceção que a pasta não existe. É assim com Java, C, python, PHP, Go, Assembler...
...
# Aqui estamos usando o filtro do dplyr em sintaxe nova. Os caracteres |> juntos
# Denotam uma instrução "pipe" ou seja, passe o resultado do ultimo comando
# para o comando seguintes.
# Nesse caso, vamos renomear uma coluna que tem espaços para um nome mais 
# programming friendly: sem espaços e minúsculos.
df_sjr <- df_sjr |>
  rename(
...
# Na ultima funçao, a mutate, ela vai vriar a coluna freq como resulado do cáculo do n / sum(n)
# para cada agrupamento do count. Isso permite que transformemos 39 em 200 vire 19.5 e formatamos em %
quart_counts <- df_sjr |>
