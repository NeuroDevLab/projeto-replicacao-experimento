...
numero_grupos <- 3 # o grupo KO se subdivide em: Classe 1, Classe 2 e Classe 3 quanto á má formação por CRISPR
cohen_f <- 0.25 # O f de Cohen é o quão grande são as medidas de dispersão entre as médias
                # pois temos 2 pools (k=2) em relaçao a variabilidade dentro dos grupos

# Temos dois grupos: KO e WT com 3 grupos de teste com aproximadamente 40 animais.
# Isso nos permite apenas o método com ANOVA, pois o Test t é para até dois grupos.
# Temos dois grupos e nos grupos temos 3 pools de 40 aniais, o nosso n é 3, só temos
# ANOVA para o cálculo amostra.
d <- pwr.anova.test(k = numero_grupos, f = cohen_f, sig.level = nivel_confianca, power = poder_estatistico)
plot(d)
```
...
# Número de animais que usaremos no estudo.
prj_n <- 153
prj_groups <- c("KO", "WT")
```
...
# Tamanho de amostra com base no tamanho de efeito do artigo.
